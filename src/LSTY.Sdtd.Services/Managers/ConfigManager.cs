...
namespace LSTY.Sdtd.Services
{
    ...
    {
        private const string _functionConfigRelativePath = "config/FunctionConfig.xml";
        ...
        private static void CreateFileWatcher()
        {
            try
            {
                ...
                _fileWatcher = new FileSystemWatcher("config", "functionConfig.xml");
        ...
        {
            if (File.Exists(_functionConfigRelativePath) == false)
            {
                return;
            }
            ...
            try
            {
                ...
                using (FileStream fs = new FileStream(_functionConfigRelativePath, FileMode.Open, FileAccess.Read))
                {
                    doc.Load(fs);
                }
        ...
        public static void Save<T>(T obj) where T : IFunction
        {
            ...
            lock (_lockObj)
            {
                try
                {
                    ...
                    using (FileStream fs = new FileStream(_functionConfigRelativePath, FileMode.OpenOrCreate, FileAccess.Write))
                    {
                        contextDoc.Save(fs);
                    }
